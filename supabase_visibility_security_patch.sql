...
CREATE POLICY "Signals are public" ON recipe_signals FOR
SELECT USING (is_public = true);
-- 3. Enable Realtime for this table
ALTER PUBLICATION supabase_realtime
ADD TABLE recipe_signals;
-- 4. Trigger to Sync `recipes` -> `recipe_signals`
CREATE OR REPLACE FUNCTION sync_recipe_signal() RETURNS TRIGGER SECURITY DEFINER AS $$ BEGIN IF (TG_OP = 'INSERT') THEN
INSERT INTO recipe_signals (recipe_id, is_public)
VALUES (NEW.id, NEW.is_public);
RETURN NEW;
ELSIF (TG_OP = 'UPDATE') THEN
INSERT INTO recipe_signals (recipe_id, is_public)
VALUES (NEW.id, NEW.is_public) ON CONFLICT (recipe_id) DO
UPDATE
SET is_public = NEW.is_public,
    updated_at = now();
RETURN NEW;
ELSIF (TG_OP = 'DELETE') THEN
DELETE FROM recipe_signals
...
INSERT
    OR
UPDATE
    OR DELETE ON recipes FOR EACH ROW EXECUTE FUNCTION sync_recipe_signal();
-- 5. Backfill existing data
INSERT INTO recipe_signals (recipe_id, is_public)
SELECT id,
    is_public
FROM recipes ON CONFLICT (recipe_id) DO
UPDATE
